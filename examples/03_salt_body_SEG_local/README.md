...
Both variants were updated to use the repo-local solver and both were validated . The 1D and 3D jobs were submitted on `<your-partition>` and both reached the Newmark time loop.
...
- `run1D_slurm.sbatch`, `run3D_slurm.sbatch`: Slurm wrappers.
...
```bash
sbatch run1D_slurm.sbatch
sbatch run3D_slurm.sbatch
```
