...
Both variants were made runnable with repo-local launchers and validated . The 1D and 3D crust jobs were each submitted on `<your-partition>` and both reached the Newmark time loop.
...
- `run_with_1d_crust_slurm.sbatch`, `run_with_3d_crust_slurm.sbatch`: Slurm wrappers.
...
4. If you use environment modules, adapt the module names in the shell launchers to match your site.
...
```bash
sbatch run_with_1d_crust_slurm.sbatch
sbatch run_with_3d_crust_slurm.sbatch
```

Each batch wrapper writes to `output_slurm_<jobid>/` inside the matching simulation directory.
