...
The following subcases were validated :

- `example_input_cartesian`: tested and reached the Newmark time loop.
- `example_single_plume`: tested and reached the Newmark time loop.
...
- launches the solver with `run.sh` or `run_slurm.sbatch`.
...
- launches the solver from `run.sh` or `run_slurm.sbatch`,
...
Each subcase writes its own `output/` or `output_slurm_<jobid>/` directory inside the subcase folder.
