...
# top_dir = r'/Users/username/Projects/Documents/AxiSEM3D/examples/11_wave_visualization_Moon'  # to adapt with user's directory
top_dir = os.getcwd()
folder = 'simu3D'
...
# Background image
texture_name = 'Moon.jpg'  # It has to be a .jpg such as 'Moon.jpg' for the NASA image

################# END PARAMETERS #################

# output.txt file
output_name = 'output.txt'
output_file = os.path.join(os.getcwd(), run, output_name)
target_line = '============================ Sources ==========================='
...
def find_lines_number(
        filename,
        target_line,
        lines_to_skip
):
    """
    Function to retrieve the number of lines from a file

    Parameters:
        filename (string): File name to be read
        target_line (string): Target line
...
def find_coordinates(
    ...
    """
    Function to retrieve the coordinates of the stations from a file
...
def sismometer_observation(
    ...
    """
    Function to retrieve the data and time from the sismometers data

    Parameters:
        top_dir (string): Top directory
        run (string): Run title of the simulation
        folder (string): Folder name
        short_title (string): Short title of the simulation
        new_station_group (string): New station group
        station_group (string): Station group
    """
    ...
    fn_file = os.path.join(top_dir, run, folder, 'output', 'stations', station_group,
                           'axisem3d_synthetics.nc.rank_all.nc')

    ds = nc4.Dataset(fn_file)

    # Open the original NetCDF4 file
    dst_file = os.path.join(top_dir, run, folder, 'output', 'stations', new_station_group,
                            'axisem3d_synthetics.nc.rank_all.nc')
        ...
        if var_name == 'data_time' or var_name == 'list_channel':
            ...
            for dim_name in variable.dimensions:
                if dim_name not in dst.dimensions:
                    dst.createDimension(dim_name, len(ds.dimensions[dim_name]) if not ds.dimensions[
                        dim_name].isunlimited() else None)
            ...
            if var_name == 'list_lat' or var_name == 'list_lon':
                ...
                new_var = dst.createVariable(var_name, variable.datatype,
                                             ('new_dim_station', 'new_dim_station_str_length'))
                ...
                new_var = dst.createVariable(var_name, variable.datatype,
                                             ('new_dim_station', 'new_dim_channel', 'new_dim_time'))
    ...
    original_stream_run, st_rank_all = get_all_streams_from_netcdf(top_dir=top_dir,
                                                                   run=run, short_title=short_title,
                                                                   station_group=new_station_group,
                                                                   station_file=station_file,
                                                                   source_name=source_name, folder=folder,
...
def animate_pyvista_png(
        ...
        run_title: str = None,
        new_station_group: str = None,
        cmap: str = None,
        texture_name: str = None
):
    """
    Function that creates the images related to the previously created meshes
    ...
    Parameters:
        ...
        station_group (string): Station group name
        station_file (string): Station file name
        run_title (string): Run title
        new_station_group (string): New station group name
        cmap (string): Cmap name
    ...
    event_pos = (5 * R * np.cos(event_lat) * np.cos(event_lon), 5 * R * np.cos(event_lat) * np.sin(event_lon),
                 5 * R * np.sin(event_lat))
    ...
    # plotter.add_background_image(image_path) # It is possible to add background stars by uncommenting this line
    ...
    for mesh_file in mesh_files:
        ...
        text_actor = plotter.add_text('{:.02f} s'.format(data_time[number]), position='lower_left', color='white',
        ...
        sphere.active_texture_coordinates[:, 0] = 0.5 + np.arctan2(-sphere.points[:, 0], sphere.points[:, 1]) / (
                    2 * np.pi)
        ...
        plotter.add_point_labels(sch, "My Labels", point_size=25,
                                 italic=True, font_size=60, text_color='white', point_color='black', shape_opacity=0,
                                 render_points_as_spheres=True, always_visible=True)

        # Add a flash at the moment of the impact (To be uncommented if needed)
...
if __name__ == '__main__':
    number = animate_pyvista_png(top_dir=top_dir,
                                 ...
                                 run_title=run_title,
                                 new_station_group=new_station_group,
                                 cmap=cmap,
