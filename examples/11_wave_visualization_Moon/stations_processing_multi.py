...
# Visible surface of the moon in m2
moon_visible_surf = 13000000
...
def processing(itime,top_dir,run,element_name):
    ...
    for ch_inc in include_channels:
        ...
        for wave_dim, channel1 in enumerate(ds['list_channel']):
            ...
            if len(points) >= 6:
                ...
                # if the surface is lower than the visible surface of the moon then we add a non zero threshold
...
if __name__ == '__main__':
    ...
    with mp.Pool(processes=mp.cpu_count()) as pool:
        pool.starmap(processing, [(itime,top_dir,run,element_name) for itime in range(0,ntime)])
