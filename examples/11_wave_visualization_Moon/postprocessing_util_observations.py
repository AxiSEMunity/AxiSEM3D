...
from impact_simulations.util import linear_interpolation, remove_negative_ones, remove_negative_ones_trace
            ...
            for station in network:
                if ((origin_time >= station.start_date) and (origin_time <= station.end_date)):
                    ...
                    distance, azimuth_A_B, azimuth_B_A = gps2dist_azimuth(
                        event_latitude, event_longitude, station.latitude, station.longitude, MOON_RADIUS_IN_KM * 1000.,
                        MOON_FLATTENING)
                    ...
                    try:
                        client = Client("IRIS")
                        st = client.get_waveforms('XA', station.code, '*', channel, origin_time - 1800,
                                                  origin_time + 5400)
            ...
            for station in network:
                if ((impact_time >= station.start_date) and (impact_time <= station.end_date)
                    ...
                    if event_elevation is not None:
                        ...
                        elevation_diff = ''
                    print(station.code, distance_in_km, distance_in_degree, elevation_diff, event_elevation,
                          station.elevation)
                    degrees.append(distance_in_degree)
                    ...
                    #                     TODO
                    try:
                        st = client.get_waveforms('XA', station.code, '*', channel, impact_time - 1800,
                                                  impact_time + 5400)
                    except FDSNNoDataException:
        ...
        if event.event_type == 'crash':
            ...
            for sta in ['S12', 'S14', 'S15', 'S16']:
                ...
                for tr in event_stream:
                    ...
                    distance, azimuth_A_B, azimuth_B_A = gps2dist_azimuth(
                        event_latitude, event_longitude, station_latitude[sta], station_longitude[sta],
                        MOON_RADIUS_IN_KM * 1000., MOON_FLATTENING)
                    ...
                    if model_taup is not None:
                        arrivals = model_taup.get_travel_times(source_depth_in_km=0,
...
def plot_spectrogram_obs(original_trace1, title=None, startsecond=0, endsecond=1800, clip=[0.0, 1.0],
                         remove_response=False, freqmin=0.3, freqmax=0.9, inv=None):
    ...
    if remove_response:
        ...
        original_trace1_copy.remove_response(inventory=inv, pre_filt=pre_filt, zero_mean=True, taper=True,
                                             output="DISP",
    ...
    log = True
    fig = original_trace1_copy.spectrogram(show=False, log=log, title=title, wlen=10,
                                           samp_rate=original_trace1_copy.stats.sampling_rate, clip=clip)
    ax = fig.axes[0]
    if log:
