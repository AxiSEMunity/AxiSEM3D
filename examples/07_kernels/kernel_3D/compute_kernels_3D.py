"""
compute_kernels_3D.py – Adrian Kernel 3D
=========================================
Compute a P-wave vp sensitivity kernel (banana-doughnut) and a Moho
topography interface kernel from a completed AxiSEM3D **3D** forward
simulation that uses S362ANI volumetric heterogeneity (×2 exaggeration)
combined with a Gaussian Moho topography (±5 km, StructuredGridG3D).
...
Kernel computation is identical to the 1D companion (``compute_kernels.py``)
because AxiSEM3D stores element output in **reference** coordinates even for
3D runs.  The PRT transformation modifies the physics (Jacobians, mass
matrices) internally; coordinates stored in the output are always referenced
to the undeformed background mesh.  Therefore the constant ``MOHO_RADIUS``
is the correct evaluation radius in both 1D and 3D cases.

The Moho kernel plot additionally shows contour lines of the Moho
topography from ``--topography`` (a NetCDF generated by
``create_moho_topography.py``).  This provides visual confirmation that the
3D boundary kernel structure spatially correlates with the undulation pattern.

Usage
-----
    python compute_kernels_3D.py [options]
...
-------
    ...
    --resolution N        Grid resolution for the kernel slice (NxN points)
                          (default: 200)
    --topography FILE     Path to the Moho topography NetCDF file used for
                          plot overlay (default: input_forward/moho_topography.nc).
                          Used for visualisation only — not for kernel computation.

Outputs (written to --output dir)
----------------------------------
    vp_kernel.h5    – vp kernel data in HDF5 format (reload with SliceMesh)
    vp_kernel.png   – quick-look figure
    moho_kd.h5      – Moho interface kernel in HDF5 format
    moho_kd.png     – Moho kernel figure with topography contour overlay
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Compute a vp sensitivity kernel and Moho interface kernel "
            "from an AxiSEM3D 3D forward run (S362ANI + Moho topography)."
        )
    )
    ...
    p.add_argument("--topography",
                   default="input_forward/moho_topography.nc",
                   help=(
                       "Path to Moho topography NetCDF file for plot overlay "
                       "(default: input_forward/moho_topography.nc). "
...
def main():
    ...
    # Element output is in reference coordinates — constant radius is correct
    # even for 3D runs with undulated Moho geometry.
    ...
    # Load Moho topography NetCDF and overlay contour lines on the kernel plot.
    # This is purely for visualisation; kernel computation uses constant radius.
    topo_path = args.topography
    _contour_added = False
    if os.path.isfile(topo_path):
        try:
            import netCDF4 as _nc
            with _nc.Dataset(topo_path, "r") as _ds:
                _topo_lat = _ds.variables["latitude"][:]   # shape (n_lat,)
                _topo_lon = _ds.variables["longitude"][:]  # shape (n_lon,)
                _topo_und = _ds.variables["undulation_MOHO"][:]  # (n_lat, n_lon), metres

            # Restrict to the kernel lon/lat window for clarity
            _lat_mask = (
                (_topo_lat >= moho_lat_deg[0] - 1.0)
                & (_topo_lat <= moho_lat_deg[-1] + 1.0)
            )
            _lon_mask = (
                (_topo_lon >= moho_lon_deg[0] - 1.0)
                & (_topo_lon <= moho_lon_deg[-1] + 1.0)
            )
            _topo_lat_sub = _topo_lat[_lat_mask]
            _topo_lon_sub = _topo_lon[_lon_mask]
            _topo_und_sub = _topo_und[np.ix_(_lat_mask, _lon_mask)]

            _TOPO_LON, _TOPO_LAT = np.meshgrid(_topo_lon_sub, _topo_lat_sub)

            # Contour levels in metres (convert to km for labels)
            _und_max_m = float(np.nanmax(np.abs(_topo_und_sub)))
            if _und_max_m > 0.0:
                _n_levels = 6
                _levels_m = np.linspace(
                    -_und_max_m * 0.9, _und_max_m * 0.9, _n_levels
                )
                _cs = ax.contour(
                    _TOPO_LON, _TOPO_LAT, _topo_und_sub,
                    levels=_levels_m,
                    colors="black", linewidths=0.8, linestyles="solid",
                    alpha=0.7,
                )
                # Label contours in km
                _fmt = {lv: f"{lv / 1000.0:.1f} km" for lv in _levels_m}
                ax.clabel(_cs, fmt=_fmt, fontsize=6, inline=True)
                _contour_added = True
                print(f"  Topography contours overlaid from: {topo_path}")
        except Exception as _e:
            print(f"  WARNING: could not load topography for overlay: {_e}")
    else:
        print(f"  Topography file not found, skipping overlay: {topo_path}")
    ...
    _topo_note = " + Moho topo contours" if _contour_added else ""
    ax.set_title(
