...
// constructor
Crust1O3D::Crust1O3D(const std::string& modelName,
    double waterDensity,
    bool includeIceAsWater,
    bool ellipticity,
    int gaussianOrder,
    double gaussianDev) :
    OceanLoad3D(modelName), mWaterDensity(waterDensity), mIncludeIceAsWater(includeIceAsWater),
    mEllipticity(ellipticity), mGaussianOrder(gaussianOrder), mGaussianDev(gaussianDev) {
  // read raw data
