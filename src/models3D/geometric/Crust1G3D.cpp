...
// constructor
Crust1G3D::Crust1G3D(const std::string& modelName,
    double rSurf,
    double rMoho,
    double rBase,
...
bool
Crust1G3D::getUndulation(const eigen::DMatX3& spz, eigen::DColX& undulation) const {
  // compute grid coords
  eigen::DMatX3 crdGrid =
      coordsFromMeshToModel(spz, false, false, mEllipticity, false, false, false, mModelName);
  int nCardinals = (int)spz.rows();
...
double
Crust1G3D::getUndulationPoint(double r, double lat, double lon, double rElemCenter) const {
